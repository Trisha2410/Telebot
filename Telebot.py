...
# Using the model "chatbot_model.h5" predicting the tag for the input sentence
    ...
    for i, r in enumerate(res):
        if r > ERROR_THRESHOLD:
           ...
           for r in results:
               ...
               print("\n\tReturn_list passed to chatbot response is ", return_list)
    else:
        return_list.append({"intent":"noanswer", "probability": str(10)})
    return return_list
...
#This functions calls predict function to find out which is the most probable tag.
#And then checks the "tag" and calls functions for DB activities and finally sends back the response to the send()
def chatbot_response(msg):
    profanity_reply = ["Please mind your language.", "Think before you write.",
                       "We will extract your Ip address and launch a complaint.",
                       "Why are you using foul language", "Please behave,this is not how learned people talk."]
    pre_profanity = predict([msg])
    predict_probability = predict_prob([msg])
    print("pre_profanity", pre_profanity[0])
    print("predict_probality", predict_probability)
    if predict_probability[0] > 0.35:
        result = random.choice(profanity_reply)
        print("predict_probality", predict_probability)
        print("res in chatbot response -->", result)
    else:
        ints = predict_class(msg, model)
        # print("message in chatbot response", msg)
        # print(f"The chatbot_response ints values are : \t", ints[0]['intent'])
        if (ints[0]['intent'] == "prepaid" or ints[0]['intent'] == "postpaid"):
            db_insertplantype(ints[0]['intent'])
        if (ints[0]['intent'] == 'budget'):
            db_insertbudget(msg)
            print("Budget tag encountered, sent to db_insertbudget()")
        if (ints[0]['intent'] == 'data_limit'):
            ...
            result1 = "Option 1: "
            result2 = sep + "\n\n" + "Option 2: "
            init_txt = "Thank you!, Please find the top result for your search criteria.\n\n"
            initial_text = "Thank you!, Please find the top 2 options for your search criteria.\n\n"
            ...
            if len(sql_res) == 2:
                ...
                # print("result of the sql is :", result[0])
            elif len(sql_res) == 1:
                # print("sql_res[0][:6]",sql_res[0][0])
...
# Retrieving the message entered by the customer in the entrybox and calling the chatbot_response()
def send(*args):
    ...
    if msg != '':
        ...
        # ChatLog.tag_add("start1",1.0,'end')#faa21f
        ChatLog.tag_config('start1', foreground="aqua", font=("Courier New", 14))
        res = chatbot_response(msg)
        if res == "":
            res = "Sorry for the inconvenience, looks like i missed something. Please type again!"
            ChatLog.insert(END, "Bot" + " " + " : " + res + '\n\n', 'start')

            ChatLog.tag_config("start", foreground="chartreuse1", font=("Courier New", 14))

        else:
            ChatLog.insert(END, "Bot" + " " + " : " + res + '\n\n', 'start')

            ChatLog.tag_config("start", foreground="#faa21f", font=("Courier New", 14))
...
logo=Label(base,image = tkimage,bg="black")
